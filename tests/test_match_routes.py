...
def test_get_matches_for_tournament_no_matches(client, user_token_headers, test_tournament):
    ...
    response = client.get(f'/matches?tournament_id={test_tournament.id}', headers=user_token_headers)
...
def test_create_and_get_success(
    ...
    match_id = data['id']
    response = client.get(
        f'/matches?tournament_id={test_tournament.id}',
        headers=user_token_headers
    )
    data = response.json()
...
def test_match_competitors_success(client, user_token_headers, test_tournament):
    ...
    for name in team_names:
        ...
        assert response.status_code == 201, response.text
    response = client.get(
        f'/competitors?tournament_id={test_tournament.id}',
        headers=user_token_headers
    )
